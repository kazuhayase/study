...
public: 
  ...
  Key( int win=0, int lose=0, int sun=0 )
   : win_(win), lose_(lose), sun_(sun){}

  int win() const { return win_; }
  int lose() const { return lose_; }
  int sun() const {return sun_;}
...
// key = (#win,#lose,sun), value = probability
typedef pair<Key, double> probPair;
multimap <Key, double> probMap[MAX_K * 2+1];
//map <Key, double> probMap;
 ...
 void init(){
   ...
   Key key_init = Key(0,0,(int)(pi*1000));
 ...
 void dp(){
   ...
   for(int j=1; j < k*2+1; j++){
     ...
     multimap <Key, double>::iterator it = probMap[j-1].begin();
	...
	double winP = ((double)sun * ps + (double)(1000-sun) * pr)/1000;

	Key keyLoseDown = Key(win, lose+1, max(sun-(int)(pd*1000),0));
	double valueLoseDown = value * (1-winP) *pl;

	Key keyLoseSame = Key(win, lose+1, sun);
	double valueLoseSame = value * (1-winP) * (1-pl);

	Key keyWinUp = Key(win+1, lose, min(sun+(int)(pu*1000), 1000));
	double valueWinUp = value * winP *pw;

	Key keyWinSame = Key(win+1, lose, sun);
	...
	multimap <Key, double>::iterator nit;
