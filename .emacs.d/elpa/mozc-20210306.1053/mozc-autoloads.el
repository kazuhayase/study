...
Create a new empty keymap and return it." (make-hash-table :size 128 :test (function eq)))
...
This indicator is not shown when you don't use LEIM." :type (quote (choice (const :tag "No indicator" nil) (string :tag "Show an indicator"))) :group (quote mozc))

(defun mozc-leim-register-input-method nil "\
Register function `mozc-mode' as an input method of LEIM.
Do nothing if LEIM is not available." (if (fboundp (function register-input-method)) (register-input-method "japanese-mozc" "Japanese" (lambda (input-method) (let ((new (quote deactivate-current-input-method-function)) (old (quote inactivate-current-input-method-function))) (set (if (boundp new) new old) (lambda nil (mozc-mode nil)))) (mozc-mode t)) mozc-leim-title "Japanese input method with Mozc.")))
