...
def do_test(N, S, T, a, b, __expected):
    startTime = time.time()
    instance = FromToDivisibleDiv2()
    exception = None
    try:
...
def run_tests():
    sys.stdout.write("FromToDivisibleDiv2 (1000 Points)\n\n")
    ...
    with open("FromToDivisibleDiv2.sample", "r") as f:
        while True:
            label = f.readline()
            ...
            passed += do_test(N, S, T, a, b, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1474906365
    PT, TT = (T / 60.0, 75.0)
