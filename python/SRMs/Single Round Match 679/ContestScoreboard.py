...
def do_test(scores, __expected):
    startTime = time.time()
    instance = ContestScoreboard()
    exception = None
    try:
        __result = instance.findWinner(scores);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("ContestScoreboard (500 Points)\n\n")
    ...
    with open("ContestScoreboard.sample", "r") as f:
        while True:
            label = f.readline()
