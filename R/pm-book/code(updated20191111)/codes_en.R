...
########################################
#####Initialization (For not beginners to R to install all required packages beforehand)
rm(list = ls())
required <- c(
...
detailedCV <- function(data, cv.pred, name = NULL) {
  total_err <- 0 ## "err" means "error for validation data"
  total_res <- 0 ## "res" means "residual for training data"
  df <- data.frame() ## Data frame to show results
  y <- vector()
  yhat <- vector()
  for (i in 1:nfolds) {
    cv.train <- data[-cv.no[[i]], ] 
