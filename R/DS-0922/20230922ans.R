...
for(i in 1:p){
  plot(train[, i], train$y, pch =".", xlab = colnames(train)[i])
}

# size, rooms, year with lowess curves
for(i in 1:3){
  plot(train[, i], train$y, xlab = colnames(train)[i])
...
##Modeling##
set.seed(SEED)
rndmFrst <- randomForest(y ~ ., data = train, importance = TRUE)
...
##EDA##
# Importance
imp <- sort(randomForest::importance(rndmFrst, type = 1, 
                                     scale = FALSE)[, 1], decreasing = TRUE)
barplot(imp, names.arg = rownames(imp))

# PDP
for (i in 1:3) {
  xname <- colnames(xy)[i]
  pdp <- partial(object = rndmFrst,
                 pred.var = c(xname))
...
###LASSO###
library(glmnetUtils)
#lambdas <- 0.1 ^ seq(0, 2, length.out = 25)
(formu <- formula(lmInt))
##textbook#p.144##
set.seed(SEED)
lsso <- cv.glmnet(formu, 
                  data = train, 
                  alpha = 1)#, 
#                  lambda = lambdas)
plot(lsso)
